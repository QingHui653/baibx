...
<body>
   ...
   <!-- <iframe frameborder="0" width="100%" height=506px; src="top.html" name="top"></iframe> -->
    ...
    <c:forEach items="${pageSupport.result}" var="news" varStatus="NA" >
     ...
     <div class="title">${fn:substring(news.title, 0, 13)} </div>
       ...
       <table width="100%" class="sxye">
    	<tr>
    		...
    		<c:when test="${pageSupport.startPage!=1}">
    			...
    			<a href="shouyenews.action?pageNo=${pageSupport.prePage}">上一页</a>
    					</c:when>	
    	</c:choose>
    			</td>
    	...
    	<c:choose>
    		<c:when test="${pageSupport.nextPage<=pageSupport.totalPages}">
    			...
    			<a href="shouyenews.action?pageNo=${pageSupport.nextPage}">下一页</a>
    			</c:when>
    			</c:choose>
    			</td>
