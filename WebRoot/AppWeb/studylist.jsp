...
<link href="<%=path%>/AppWeb/css/slist.css" rel="stylesheet" type="text/css">
<title>保险资料</title>
</head>
     ...
     <div class="Column">
		<span class="Arrow">
   		...
   		<table align="center" class="table" width="100%">
        	...
        	<tr>
            	<td>
            	<a href=" <c:url value='apstudyDetail.action?id=${Information.id}&pageNo=1'/>" >
         		<div>${fn:substring(Information.title, 0, 22)} <div class="right">${fn:substring(Information.time, 0, 11)}</div></div>
    	...
    	<tr>
    		...
    		<c:when test="${pageSupport.startPage!=1}">
    			...
    			<a href="apstudyslist.action?PageNo=${pageSupport.prePage}&type=${pageSupport.result[0].type}">上一页</a>
    					</c:when>	
    	</c:choose>
    			</td>
    	...
    	<c:choose>
    		<c:when test="${pageSupport.nextPage<=pageSupport.totalPages}">
    			...
    			<a href="apstudyslist.action?PageNo=${pageSupport.nextPage}&type=${pageSupport.result[0].type}">下一页</a>
    			</c:when>
    			</c:choose>
    			</td>
